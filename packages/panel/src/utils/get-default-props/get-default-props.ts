...
export function isRequired(validator: Validator<unknown>): boolean {
  ...
  return requiredValidators.includes(
    validator as (typeof requiredValidators)[number]
  );
}

/** Returns default for specified remote component */
export async function getDefaultPropsForRemoteComponent(
...
export function getDefaultProps<T extends object>(
  propTypes: ValidationMap<T>
): T {
  const entries = (
    Object.entries(propTypes) as [keyof T, Validator<ValidatorData>][]
...
export function getDefaultValue<T extends ValidatorData>(
...
export function getValidatorType<T extends ValidatorData>(
  validator: Validator<T>
): ValidatorType {
  switch (validator) {
