...
import { loadComponent, RemoteComponentData } from '../load-component';
...
export function isRequired(validator: Validator<unknown>): boolean {
  const requiredValidators = [PropTypes.string.isRequired, PropTypes.number.isRequired];

  return requiredValidators.includes(validator as (typeof requiredValidators)[number]);
}

/** Returns default for specified remote component */
export async function getDefaultPropsForRemoteComponent(componentData: RemoteComponentData) {
  const component = await loadComponent({ componentData });
  const propTypes = component.propTypes ?? {};

  return getDefaultProps(propTypes);
}

/** Returns default props for specified propTypes */
export function getDefaultProps<T extends object>(propTypes: ValidationMap<T>): T {
  const entries = (Object.entries(propTypes) as [keyof T, Validator<ValidatorData>][]).map(
    ([key, validator]) => [key, getDefaultValue(validator)]
  );
